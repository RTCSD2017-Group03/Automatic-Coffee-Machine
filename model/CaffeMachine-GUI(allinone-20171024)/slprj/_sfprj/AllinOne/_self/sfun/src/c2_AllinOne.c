  ...
  *chartInstance)
{
  ...
  c2_d_y = NULL;
  sf_mex_assign(&c2_d_y, sf_mex_create("y", &c2_c_hoistedGlobal, 11, 0U, 0U, 0U,
    0), false);
  sf_mex_setcell(c2_y, 2, c2_d_y);
  c2_d_hoistedGlobal = chartInstance->c2_ClearIntegratorEventEventCounter;
  c2_e_y = NULL;
  sf_mex_assign(&c2_e_y, sf_mex_create("y", &c2_d_hoistedGlobal, 7, 0U, 0U, 0U,
    0), false);
  ...
  switch (chartInstance->c2_is_c2_AllinOne) {
   case c2_IN_Finish:
    ...
    if (CV_EML_COND(3, 0, 0, chartInstance->c2_temporalCounter_i1 >= 5) &&
        CV_EML_COND(3, 0, 1, *chartInstance->c2_SettingAmount == 0.0)) {
...
static void chart_debug_initialize_data_addresses(SimStruct *S)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S)) {
      ...
      {
        _SFD_SET_DATA_VALUE_PTR(1U, (void *)chartInstance->c2_SettingAmount);
        _SFD_SET_DATA_VALUE_PTR(3U, (void *)chartInstance->c2_RemainLiquid);
        _SFD_SET_DATA_VALUE_PTR(2U, (void *)chartInstance->c2_OutLiquid);
        _SFD_SET_DATA_VALUE_PTR(0U, (void *)&chartInstance->c2_TargetAmount);
      }
    }
  }
}
