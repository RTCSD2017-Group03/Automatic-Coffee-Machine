...
static void c3_durationReferenceTimeUpdater(SFc3_AllinOneInstanceStruct
  *chartInstance);
...
static void chart_debug_initialize_data_addresses(SimStruct *S)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S)) {
      ...
      {
        _SFD_SET_DATA_VALUE_PTR(1U, chartInstance->c3_SettingAmount);
        _SFD_SET_DATA_VALUE_PTR(3U, chartInstance->c3_RemainLiquid);
        _SFD_SET_DATA_VALUE_PTR(2U, chartInstance->c3_OutLiquid);
        _SFD_SET_DATA_VALUE_PTR(0U, &chartInstance->c3_TargetAmount);
      }
    }
  }
}
