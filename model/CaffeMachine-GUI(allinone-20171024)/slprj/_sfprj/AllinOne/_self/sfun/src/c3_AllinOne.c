  ...
  *chartInstance)
{
  ...
  c3_i_y = NULL;
  sf_mex_assign(&c3_i_y, sf_mex_create("y", c3_h_u, 11, 0U, 1U, 0U, 1, 2), false);
  ...
  const mxArray *c3_st)
{
  ...
  *chartInstance->c3_RemainLiquid = c3_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c3_u, 0)), "RemainLiquid");
  ...
  *chartInstance->c3_ClearIntegratorEvent = c3_c_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c3_u, 2)), "ClearIntegratorEvent");
...
static void sf_gateway_c3_AllinOne(SFc3_AllinOneInstanceStruct *chartInstance)
{
  ...
  _SFD_DATA_RANGE_CHECK(*chartInstance->c3_SettingAmount, 0U);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c3_RemainLiquid, 1U);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c3_OutLiquid, 2U);
  ...
  switch (chartInstance->c3_is_c3_AllinOne) {
   case c3_IN_Finish:
    ...
    if (CV_EML_COND(3, 0, 0, chartInstance->c3_temporalCounter_i1 >= 5)) {
      if (CV_EML_COND(3, 0, 1, *chartInstance->c3_SettingAmount == 0.0)) {
   ...
   case c3_IN_Ready:
    ...
    if (c3_b_out) {
      ...
      _SFD_DATA_RANGE_CHECK(chartInstance->c3_TargetAmount, 3U);
      c3_errorIfDataNotWrittenToFcn(chartInstance, 1U, 3U, 8U, 64, 12);
      c3_hoistedGlobal = chartInstance->c3_TargetAmount;
      c3_A = c3_hoistedGlobal;
      c3_x = c3_A;
      c3_b_x = c3_x;
      c3_c_x = c3_b_x;
      *chartInstance->c3_RemainLiquid = c3_c_x / 3.0;
   ...
   case c3_IN_Transmiting:
    ...
    _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c3_c_out, 2U, c3_b_sf_marshallOut,
      c3_b_sf_marshallIn);
    c3_errorIfDataNotWrittenToFcn(chartInstance, 1U, 3U, 10U, 1, 12);
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        _SFD_SET_DATA_COMPILED_PROPS(3,SF_DOUBLE,0,NULL,0,0,0,0.0,1.0,0,0,
          (MexFcnForType)c3_sf_marshallOut,(MexInFcnForType)c3_sf_marshallIn);
        _SFD_SET_DATA_VALUE_PTR(0U, chartInstance->c3_SettingAmount);
        _SFD_SET_DATA_VALUE_PTR(1U, chartInstance->c3_RemainLiquid);
        _SFD_SET_DATA_VALUE_PTR(2U, chartInstance->c3_OutLiquid);
